...
std::string GetExePath() { return GetBundlePath() + "/Contents/MacOS"; }
...
std::string GetBundlePath() { return GetExePath(); }
#endif

std::string GetResourcePath() {
  auto const ExePath       = GetExePath();
  auto const ExePathExists = std::filesystem::exists(ExePath);
...
int main(void) {
  ...
  //
  auto const ShaderPath =
      GetResourcePath() + "/resources/shaders/glsl" + std::to_string(GLSL_VERSION) + "/julia_set.fs";
  Shader shader = LoadShader(0, ShaderPath.c_str());
  // Shader shader = LoadShader(0, TextFormat("./resources/shaders/glsl%i/julia_set.fs", GLSL_VERSION));

  TraceLog(LOG_DEBUG, "ShaderPath: %s", ShaderPath.c_str());
  TraceLog(LOG_DEBUG, "ExePath   : %s", GetExePath().c_str());
  TraceLog(LOG_DEBUG, "BundlePath: %s", GetBundlePath().c_str());
  TraceLog(LOG_DEBUG, "ResourcePath: %s", GetResourcePath().c_str());
